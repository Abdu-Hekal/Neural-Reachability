...
class Transform:
    ...
    def transform_file(self, name, angle, xd, yd):
        f = open(name, "r")
        xs = []
        ys = []
        for line in f:
            if len(line) > 1:
                ...
                if len(xs) == 9:
                    self.transform(xs, ys, angle, xd, yd)
                xs = []
                ys = []

    def plot_oct(self, xs, ys):
...
if __name__ == '__main__':
    plt.figure()
    transform = Transform()
    transform.transform_file(name="data", angle=3.14, xd=2, yd=2)
    plt.show()
